...
const { factory } = require("mathjs");

const createIntegral = factory(
  "integrate",
  [
    "parse",
    "simplify",
  ...
  function ({
        ...
        switch (expr.type) {
          ...
          default:
            throw new Error(
              "Node type '" +
                expr.type +
                "' is currently unsupported in isConstant."
            );
        }
      }
    };
    ...
    function getNumericValue(expr) {
      ...
      function toNumber(expr) {
        if (
          ...
          expr.type === "ConstantNode" &&
          (!expr.valueType || expr.valueType === "number")
        ) {
          return +expr.value;
        } else {
          return undefined;
    ...
    integral.rules = [
      ...
      function (expr, context, subIntegral) {
        if (expr.type === "OperatorNode" && expr.op === "*") {
          ...
          if (factors.length > expr.args.length) {
            return subIntegral(
              new OperatorNode("*", "multiply", factors),
              context,
              "product flattening"
            );
          }
      ...
      function (expr, context) {
        if (expr.type === "SymbolNode" && expr.name === context.variable.name) {
          return new OperatorNode("*", "multiply", [
            ...
            new OperatorNode("^", "pow", [expr.clone(), new ConstantNode(2)]),
          ]);
        }
      },
      ...
      function (expr, context, subIntegral) {
        if (expr.type === "OperatorNode" && expr.op === "*") {
          ...
          if (constantFactors.length !== 0) {
            ...
            var nonConstantIntegral = subIntegral(
              nonConstantNode,
              context,
      ...
      function (expr, context) {
        if (
          expr.type === "OperatorNode" &&
          expr.op === "^" &&
          expr.args[0].equals(context.variable) &&
          context.isConstant(expr.args[1])
        ) {
          // Simplify Exponent if constant
          var exponentValue = getNumericValue(expr.args[1]);
          if (exponentValue === -1) {
            return new FunctionNode("log", [
              new FunctionNode("abs", [context.variable.clone()]),
            ]);
          } else {
            return new OperatorNode("*", "multiply", [
              new OperatorNode("/", "divide", [
      ...
      function (expr, context) {
        if (expr.type === "OperatorNode" && expr.op === "^") {
          if (
            context.isConstant(expr.args[0]) &&
            expr.args[1].equals(context.variable)
          ) {
            return new OperatorNode("/", "divide", [
              expr,
          ...
          if (base.type === "OperatorNode" && base.op === "*") {
            return subIntegral(
              new OperatorNode(
                "*",
                "multiply",
                base.args.map(function (baseChild) {
      ...
      function (expr, context, subIntegral) {
        if (expr.type === "OperatorNode" && expr.op === "*") {
          ...
          if (wasChange) {
            var int = subIntegral(
              new OperatorNode("*", "multiply", processedArgs),
              context,
              "removing unary +/- from factors"
            );
            if (int) {
              return isTotalPositive
                ? int
                : new OperatorNode("-", "unaryMinus", [int]);
            }
          }
        }
      },
      ...
      function (expr, context, subIntegral) {
        if (expr.type === "OperatorNode" && expr.op === "*") {
          ...
          for (var i = 0; i < expr.args.length; i++) {
            if (
              expr.args[i].type === "OperatorNode" &&
              (expr.args[i].op === "+" || expr.args[i].op === "-")
            ) {
              sumNode = expr.args[i];
              otherFactors = expr.args.filter(function (expr, index) {
                return index !== i;
              });
              break;
            }
          ...
          if (sumNode !== null) {
            var newTerms = sumNode.args.map(function (term) {
              return new OperatorNode(
                "*",
                "multiply",
                otherFactors.concat([term])
              );
            });
            return subIntegral(
              new OperatorNode(sumNode.op, sumNode.fn, newTerms),
              context,
      ...
      function (expr, context, subIntegral) {
        ...
        if (uniqueParent !== null && uniqueParent.type === "OperatorNode") {
          if (uniqueParent.op === "+" || uniqueParent.op === "-") {
            if (uniqueParent.args.length === 1) {
              ...
              createIntegralWrapper = function (int) {
                return new OperatorNode(uniqueParent.op, uniqueParent.fn, [
                  int,
                ]);
              };
            } else {
          ...
          if (createIntegralWrapper !== null) {
            var preIntegral = replaceNodeInTree(
              expr,
              uniqueParent,
              context.variable.clone()
            );
            ...
            if (int) {
              var backSubstituted = replaceNodeInTree(
                int,
                context.variable,
                uniqueParent
              );
        ...
        function getParentOfUniqueVariable(expr) {
          ...
          function helper(expr, parent) {
            if (context.isConstant(expr)) {
              ...
              expr.type === "SymbolNode" &&
              expr.name === context.variable.name
            ) {
              return parent;
            } else {
            ...
            if (multipliedOutChild) {
              var int = subIntegral(
                new OperatorNode(
                  "*",
                  "multiply",
                  multipliedOutChild.args.concat(
                    expr.args.slice(0, i),
                    expr.args.slice(i + 1)
                  )
                ),
        ...
        function tryMultiplyOut(expr) {
          if (
            ...
            if (
              ...
              exponentValue > 1 &&
              exponentValue <= MaxExponentExpanded
            ) {
              return new OperatorNode("*", "multiply", [
                expr.args[0],
                exponentValue === 2
    ...
    function _integral(expr, context, lastRuleComment) {
      ...
      debugComment +=
        "find integral of " + exprString + "  d" + context.variable.name;
      ...
      for (var i = 0; i < context.rules.length; i++) {
        ...
        if (result !== undefined && result !== null) {
          ...
          context.printDebug(
            "Computed: " + result.toString({ parenthesis: "all" })
          );
          context.debugIndent--;
          return result;
        }
